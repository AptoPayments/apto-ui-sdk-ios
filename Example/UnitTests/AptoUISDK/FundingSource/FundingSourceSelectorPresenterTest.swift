...
private extension FundingSourceSelectorPresenterTest {
    func givenThereAreValidFundingSourcesAvailable(_ number: Int = 3) {
        let fundingSources = (0 ... number).map { _ in dataProvider.fundingSource }
        sut.viewModel.fundingSources.send(fundingSources)
    }

    func activateFundingSourceAt(index: Int) {
