...
class StorageLocatorFake: StorageLocatorProtocol {
    ...
    func userStorage(transport _: JSONTransport) -> UserStorageProtocol {
        return userStorageFake
    }

    lazy var configurationStorageFake = ConfigurationStorageFake()
    func configurationStorage(transport _: JSONTransport) -> ConfigurationStorageProtocol {
    ...
    func financialAccountsStorage(transport _: JSONTransport) -> FinancialAccountsStorageProtocol {
    ...
    func paymentSourcesStorage(transport _: JSONTransport) -> PaymentSourcesStorageProtocol {
        PaymentSourcesStorageFake()
    }

    lazy var agreementStorageSpy = AgreementStorageSpy()
    func achAccountAgreementStorage(transport _: JSONTransport) -> AgreementStorageProtocol {
        return agreementStorageSpy
    }

    lazy var achAccountStorageSpy = ACHAccountStorageSpy()
    func achAccountStorage(transport _: JSONTransport) -> ACHAccountStorageProtocol {
        return achAccountStorageSpy
    }

    lazy var applePayIAPStorageSpy = ApplePayIAPStorageSpy()
    func applePayIAPStorage(transport _: JSONTransport) -> ApplePayIAPStorageProtocol {
        return applePayIAPStorageSpy
    }

    lazy var cardApplicationStorageSpy = CardApplicationStorageSpy()
    func cardApplicationStorage(transport _: JSONTransport) -> CardApplicationsStorageProtocol {
        return cardApplicationStorageSpy
    }

    lazy var p2pTransferStorageSpy = P2PTransferStorageSpy()
    func p2pTransferStorage(transport _: JSONTransport) -> P2PTransferProtocol {
