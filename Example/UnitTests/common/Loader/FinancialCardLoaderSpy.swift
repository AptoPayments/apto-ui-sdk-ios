...
class FinancialCardLoaderSpy: AptoPlatformFake {
    ...
    override func orderPhysicalCard(_: String, callback: @escaping (Result<Card, NSError>) -> Void) {
        orderCardCompletion.append(callback)
    }

    override func getOrderPhysicalCardConfig(_: String, callback: @escaping (Result<PhysicalCardConfig, NSError>) -> Void) {
