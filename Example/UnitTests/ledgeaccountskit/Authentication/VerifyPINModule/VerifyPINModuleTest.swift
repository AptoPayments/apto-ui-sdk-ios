...
class VerifyPINModuleTest: XCTestCase {
  private var sut: VerifyPINModule! // swiftlint:disable:this implicitly_unwrapped_optional
  ...
  override func setUp() {
    super.setUp()
    sut = VerifyPINModule(serviceLocator: serviceLocator, actionConfirmer: ActionConfirmerFake.self)
  }
  ...
  func testInitializeSetUpPresenter() {
    // Given
    platfom.nextFetchContextConfigurationResult = .success(ModelDataProvider.provider.contextConfiguration)
    let presenter = serviceLocator.presenterLocatorFake.verifyPINPresenterSpy
