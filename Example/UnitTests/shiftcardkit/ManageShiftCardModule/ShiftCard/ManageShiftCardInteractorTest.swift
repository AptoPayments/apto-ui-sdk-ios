...
class ManageShiftCardInteractorTest: XCTestCase {
  ...
  override func setUp() {
    ...
    sut = ManageShiftCardInteractor(platform: platform, card: card)
  }
  ...
  func testReloadCardCallSessionRetrievingBalance() {
    ...
    XCTAssertEqual(true, platform.lastFetchFinancialAccountRetrieveBalances)
  }
  ...
  func testReloadCardGetFinancialAccountSucceedCallbackSuccess() {
    // Given
    platform.nextFetchFinancialAccountResult = .success(dataProvider.card)
    var returnedResult: Result<Card, NSError>?
  ...
  func testLoadCardInfoCallSessionRetrievingBalance() {
    ...
    // Then
    XCTAssertTrue(platform.fetchCardDetailsCalled)
  }

  func testLoadCardInfoGetFinancialAccountFailsCallbackError() {
    // Given
    platform.nextFetchCardDetailsResult = .failure(BackendError(code: .other))
