    ...
    private func setupBindings() {
        viewModel.onCardFetched = { [weak self] card in
            ...
            guard let configuration = PKAddPaymentPassRequestConfiguration(encryptionScheme: .ECC_V2) else {
                AlertDialog.showToast(to: self,
                                      error: BackendError(code: .incorrectParameters),
            ...
            guard let paymentPassViewController = PKAddPaymentPassViewController(requestConfiguration: configuration,
                                                                                 delegate: self)
            else {
...
extension ApplePayIAPViewController: PKAddPaymentPassViewControllerDelegate {
    ...
    {
        viewModel.sendRequestData(certificates: certificates,
                                  nonce: nonce,
                                  nonceSignature: nonceSignature) { issuerResponse in
