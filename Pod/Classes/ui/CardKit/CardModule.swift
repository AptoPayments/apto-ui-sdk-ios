...
open class CardModule: UIModule {
    ...
    init(launchOptions: CardModuleLaunchOptions, initializationData: InitializationData? = nil) {
        if let initialUserData = launchOptions.initialUserData {
            userDataPoints = initialUserData.copy() as! DataPointList // swiftlint:disable:this force_cast
        } else {
            userDataPoints = DataPointList()
        }
    ...
    override public func initialize(completion: @escaping Result<UIViewController, NSError>.Callback) {
        ...
        if let cardOptions = launchOptions.cardOptions {
            platform.setCardOptions(cardOptions)
        }
        loadConfigurationFromServer { [weak self] result in
    ...
    fileprivate func prepareInitialScreen(_ completion: @escaping Result<UIViewController, NSError>.Callback) {
        if projectConfiguration.welcomeScreenAction.status == .enabled {
            let welcomeScreenModule = buildWelcomeScreenModule()
            addChild(module: welcomeScreenModule, completion: completion)
        } else {
            guard platform.currentToken() != nil else {
                // There's no user. Show the auth module
                showAuthModule(addChild: true, completion: completion)
                return
            }
            // There's a user. Check if he has already cards.
            showExistingOrNewCardModule(addChild: true, completion: completion)
        }
    }
    ...
    fileprivate func buildWelcomeScreenModule() -> ShowGenericMessageModule {
        ...
        welcomeScreenModule.onWelcomeScreenContinue = { [weak self] _ in
            guard let self = self else { return }
        ...
        welcomeScreenModule.onClose = { [weak self] _ in
            self?.close()
        }
                                    ...
                                    completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        ...
        authModule.onExistingUser = { [weak self] _, user in
        ...
        if addChild {
            authModule.onClose = { [unowned self] _ in
                self.authModule = nil
                self.close()
            }
            ...
            push(module: authModule, leftButtonMode: .close, completion: completion)
        }
    }
                                                 ...
                                                 completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        ...
        case let .manageCard(cardId: cardId):
    ...
    private func launchNewCardApplicationFlow(addChild: Bool = false,
                                              completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        showCardProductSelector(addChild: addChild, completion: completion)
    }
    ...
    private func showManageCardModule(card: Card, addChild: Bool, pushModule: Bool,
                                      completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        handleAuthentication { [unowned self] in
            let existingCardModule = self.serviceLocator
                .moduleLocator
                .manageCardModule(card: card, mode: self.launchOptions.mode)
                                   ...
                                   completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        ...
        if addChild {
            ...
            newCardModule.onFinish = { [weak self] _ in
    ...
    @objc private func didReceiveSessionExpiredEvent(_: Notification) {
    ...
    @objc private func didReceiveSessionClosedEvent(_: Notification) {
        hideLoadingView()
        hideLoadingSpinner()
        clearUserToken()
        close()
