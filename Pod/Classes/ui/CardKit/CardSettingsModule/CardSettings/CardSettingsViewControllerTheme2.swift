...
class CardSettingsViewControllerTheme2: AptoViewController, CardSettingsViewProtocol {
    ...
    private func set(lockedSwitch: Bool) {
        lockCardRow?.switcher.isOn = lockedSwitch
    }
...
private extension CardSettingsViewControllerTheme2 {
    ...
    func setupViewModelSubscriptions() {
        ...
        combineLatest(viewModel.legalDocuments,
                      viewModel.buttonsVisibility,
                      viewModel.cardShipping).observeNext { [unowned self] legalDocuments,
            buttonsVisibility, cardShipping in
            let settingsRows = [
                self.setUpAddFundsRow(showButton: buttonsVisibility.showAddFundsFeature,
                                      lastItem: buttonsVisibility.showP2PTransferFeature == false),
            ...
            let helpRows = [
                ...
                self.createCardShippingProgress(cardShipping.showShipping,
                                                title: cardShipping.title,
                                                subtitle: cardShipping.subtitle),
    ...
    // swiftlint:enable closure_parameter_position function_body_length
    ...
    func createLegalTitle(legalDocuments: LegalDocuments) -> FormRowView? {
        ...
        guard !(content.compactMap { $0 }).isEmpty else {
            return nil
        }
