...
final class P2PTransferViewModel {
    ...
    public func findRecipient(phone: String) {
        do {
            ...
            let phoneNumber = AptoPhoneNumber(countryCode: Int(truncatingIfNeeded: phone.countryCode),
                                              phoneNumber: formattedNumber)
            makeFindRecipientRequest(phone: phoneNumber)
        } catch {
            onErrorRequest?(NSError(domain: "com.aptopayment.aptocard",
                                    code: 0,
                                    userInfo: ["invalid_phone": "Unexpected invalid phone number"]))
        }
    }
