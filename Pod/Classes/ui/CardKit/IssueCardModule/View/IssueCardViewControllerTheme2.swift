...
class IssueCardViewControllerTheme2: AptoViewController {
    ...
    init(uiConfiguration: UIConfig, presenter: IssueCardPresenterProtocol) {
        ...
        legalNoticeView = ContentPresenterView(uiConfig: uiConfiguration)
        super.init(uiConfiguration: uiConfiguration)
        retryButton = ComponentCatalog.buttonWith(title: " ", showShadow: false,
    ...
    private func updateUIForState(_ state: IssueCardViewState) {
        ...
        case .done:
            hideLoadingView()
        case let .showLegalNotice(content):
            hideLoadingView()
            hideErrorState()
