...
class NotificationPreferencesPresenter: NotificationPreferencesPresenterProtocol {
    ...
    func didUpdateNotificationRow(_ row: NotificationRow) {
        guard let preferences = preferences, let channel1 = viewModel.channel1.value,
              let channel2 = viewModel.channel2.value
        else {
            return
        }
...
private extension NotificationGroup {
    subscript(index: NotificationChannel) -> Bool? {
        ...
        set(newValue) {
            switch index {
            case .push:
                channel.push = newValue
            case .email:
                return channel.email = newValue
            case .sms:
                return channel.sms = newValue
            }
        }
    }
}
