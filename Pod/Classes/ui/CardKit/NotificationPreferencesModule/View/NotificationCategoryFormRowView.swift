...
class NotificationCategoryFormRowView: FormRowView {
    ...
    init(category: NotificationCategory, showTopSeparator: Bool = false, uiConfig: UIConfig) {
        ...
        titleLabel = ComponentCatalog.sectionTitleLabelWith(text: category.title, uiConfig: uiConfig)
        explanationLabel = ComponentCatalog.timestampLabelWith(text: category.description, multiline: true,
                                                               uiConfig: uiConfig)
        checkboxesView = DoubleCheckbox(value1: category.rows[0].isChannel1Active,
