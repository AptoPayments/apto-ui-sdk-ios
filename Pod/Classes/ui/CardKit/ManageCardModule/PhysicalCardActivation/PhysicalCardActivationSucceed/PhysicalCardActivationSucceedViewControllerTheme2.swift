...
class PhysicalCardActivationSucceedViewControllerTheme2: PhysicalCardActivationSucceedViewControllerProtocol {
    ...
    init(uiConfiguration: UIConfig, card: Card, presenter: PhysicalCardActivationSucceedPresenterProtocol) {
        self.presenter = presenter
        let title = "manage_card.get_pin_nue.title".podLocalized()
        titleLabel = ComponentCatalog.largeTitleLabelWith(text: title, textAlignment: .center,
                                                          uiConfig: uiConfiguration)
        let explanation = "manage_card.get_pin_nue.explanation".podLocalized()
        explanationLabel = ComponentCatalog.formLabelWith(text: explanation, textAlignment: .center, multiline: true,
                                                          uiConfig: uiConfiguration)
        self.card = card
...
private extension PhysicalCardActivationSucceedViewControllerTheme2 {
    ...
    func createGetPinButton() {
        let buttonTitle = card.features?.setPin?.status == .enabled ?
            "manage_card.set_pin_new.call_to_action.title" : "manage_card.get_pin_nue.call_to_action.title"
        let accessibilityLabel = card.features?.setPin?.status == .enabled ?
            "Set PIN button" : "Get PIN button"
        let button = ComponentCatalog.buttonWith(title: buttonTitle.podLocalized(),
                                                 showShadow: false, accessibilityLabel: accessibilityLabel,
    ...
    func setUpChargeLabel() {
        if presenter.viewModel.showGetPinButton.value, card.features?.setPin?.status != .enabled {
