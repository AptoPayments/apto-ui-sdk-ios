...
class ManageCardModule: UIModule {
    ...
    private func registerForNotifications() {
        serviceLocator.notificationHandler.addObserver(self, selector: #selector(kycInvalidated),
    ...
    @objc private func kycInvalidated() {
        ...
        let viewController = buildKYCViewController(uiConfig, card: card)
                                                  ...
                                                  completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        ...
        physicalCardActivationModule = module
        if addChild {
            module.onFinish = { [weak self] _ in
                self?.reloadCardAndShowManageCardIfPossible { _ in }
            }
    ...
    fileprivate func showManageCardViewController(addChild: Bool = false,
                                                  completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        let viewController = buildManageCardViewController(uiConfig, card: card)
        if addChild {
    ...
    fileprivate func buildManageCardViewController(_: UIConfig,
                                           ...
                                           completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        let viewController = buildKYCViewController(uiConfig, card: card)
        if addChild {
            let leftButtonMode: UIViewControllerLeftButtonMode = mode == .standalone ? .none : .close
    ...
    fileprivate func buildKYCViewController(_: UIConfig, card: Card) -> KYCViewControllerProtocol {
...
extension ManageCardModule: ManageCardRouterProtocol {
    func update(card newCard: Card) {
        if newCard.state != .cancelled {
            ...
            // Card has been cancelled, look for other user cards that are not closed, if any.
            // If there are no non-closed
            // cards, close the SDK
            showLoadingView()
    ...
    func cardSettingsTappedInManageCardViewer() {
        ...
        shiftCardSettingsModule = module
    ...
    func balanceTappedInManageCardViewer() {
        ...
        fundingSourceSelectorModule = module
    ...
    func showTransactionDetails(transaction: Transaction) {
        ...
        present(viewController: viewController, embedInNavigationController: true) {}
    }
    ...
    func physicalActivationSucceed() {
        ...
        physicalCardModule.activationCompletion = { [weak self] in
            self?.hideLoadingView()
            self?.setCardPin()
        }
        ...
        physicalCardActivationSucceedModule = physicalCardModule
    }

    fileprivate func buildTransactionDetailsViewControllerFor(_: UIConfig,
...
extension ManageCardModule: KYCRouterProtocol {
    ...
    func kycPassed() {
        platform.fetchCard(card.accountId, forceRefresh: true,
                           retrieveBalances: false) { [weak self] result in
            guard let self = self else { return }
            switch result {
            case let .failure(error):
