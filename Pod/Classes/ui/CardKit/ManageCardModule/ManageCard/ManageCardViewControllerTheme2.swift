...
class ManageCardViewControllerTheme2: AptoViewController, ManageCardViewProtocol {
    ...
    private lazy var activationButton: UIBarButtonItem = {
        buildActivatePhysicalCardButtonItem()
    }()

    private lazy var statsButton: UIBarButtonItem = {
        buildTopBarButton(iconName: "chart-section-icon", target: self, action: #selector(showStatsTapped))
    }()

    private lazy var accountSettingsButton: UIBarButtonItem = {
        buildTopBarButton(iconName: "top_profile", target: self, action: #selector(nextTapped))
    }()
    ...
    init(mode: AptoUISDKMode, uiConfiguration: UIConfig, presenter: ManageCardEventHandler) {
        ...
        self.mode = mode
        activateCardView = ActivateCardView(uiConfig: uiConfiguration)
        balanceView = BalanceViewTheme2(uiConfiguration: uiConfiguration)
        super.init(uiConfiguration: uiConfiguration)
        mainView = ManageCardMainViewTheme2(uiConfiguration: uiConfiguration, cardStyle: nil, delegate: self)
        mainViewCellController = ViewWrapperCellController(view: mainView)
    }
...
private extension ManageCardViewControllerTheme2 {
    ...
    func setUpEmptyCaseView(iapEnabled: Bool) {
        ...
        combineLatest(presenter.viewModel.card, presenter.viewModel.cardLoaded)
            .observeNext { [weak self] card, cardLoaded in
...
private extension ManageCardViewControllerTheme2 {
    func setupViewModelSubscriptions() { // swiftlint:disable:this cyclomatic_complexity function_body_length
        ...
        viewModel.showSetCardPin.observeNext { [weak self] show in
            guard let show = show else { return }
            self?.mainView.showSetCardPin(show)
        }.dispose(in: disposeBag)
        ...
        combineLatest(viewModel.transactionsLoaded, viewModel.card)
            .observeNext { [weak self, viewModel] transactionLoaded, card in
