...
class UserDataCollectorPresenter: UserDataCollectorDataReceiver, UserDataCollectorEventHandler {
             ...
             googleGeocodingAPIKey: String?)
    {
        ...
        stepVisibility = calculateStepVisibility(userData: userData,
                                                 mode: mode,
                                                 skipSteps: skipSteps,
    ...
    fileprivate func configureNavigationFor(step: DataCollectorStep) {
        guard let stepHandler = stepsDictionary[step] else {
            return
        }
        viewController.set(title: stepHandler.title)
        if step == firstStep {
            viewController.configureLeftNavButton(mode: config.backButtonMode, uiConfig: uiConfig)
            viewController.showNavNextButton(title: "user-data-collector.next-button.title".podLocalized(),
                                             tintColor: uiConfig.iconTertiaryColor)
        } else {
            viewController.showNavPreviousButton(uiConfig.iconTertiaryColor)
            viewController.showNavNextButton(title: "user-data-collector.next-button.title".podLocalized(),
                                             tintColor: uiConfig.iconTertiaryColor)
        }
        currentStepObserving = stepHandler.valid.removeDuplicates().observeNext { [weak self] validStep in
            if validStep {
