...
final class TransferStatusView: UIView {
    ...
    private func setupView() {
        ...
        closeButton = .roundedButtonWith("load_funds.transaction.primary_cta".podLocalized(),
                                         backgroundColor: uiConfig.uiPrimaryColor ?? .blue,
    ...
    private func bind() {
        viewModel?.output.state.observeNext(with: { [weak self] state in
            switch state {
            case .idle:
                break
            case let .loaded(items, amount, softDescriptor):
    ...
    private func configureTransferState(with amount: Amount, softDescriptor: String) {
        let title = "load_funds.transaction.successfull.description"
            .podLocalized().replacingOccurrences(of: "<<VALUE>>", with: amount.absText)
        let bankDescriptionText = "load_funds.transaction.bank_description"
            .podLocalized().replacingOccurrences(of: "<<VALUE>>", with: softDescriptor)
        let learnMoreLinkText = "\n" + "load_funds.transaction.learn_more".podLocalized()
        let bankDescription = NSMutableAttributedString(string: bankDescriptionText,
                                                        attributes: [.foregroundColor: uiConfig.textPrimaryColor])
        let learnMoreLink = NSMutableAttributedString(string: learnMoreLinkText,
                                                      attributes: [.foregroundColor: uiConfig.textLinkColor])
