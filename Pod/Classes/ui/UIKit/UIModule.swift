...
open class UIModule: NSObject, UIModuleProtocol {
    ...
    var navigationController: UINavigationController? {
        get {
            if let navController = _navigationViewController {
                return navController
            }
            return parentUIModule?.navigationController
        }
        set(navController) {
            _navigationViewController = navController
        }
    }
                     ...
                     completion: @escaping Result<UIViewController, NSError>.Callback)
    {
        ...
        module.initialize { [weak self] result in
            ...
            case let .success(initialViewController):
        ...
        module.initialize { [weak self] result in
            ...
            case let .success(initialViewController):
                ...
                if presenterController != nil {
                    ...
                    presenterViewController = wself
                        .navigationController?
                        .viewControllers.last ?? UIApplication.topViewController()
                }
                if #available(iOS 13.0, *), viewController.modalPresentationStyle != .overCurrentContext {
    ...
    public func addChild(viewController: UIViewController,
                         leftButtonMode _: UIViewControllerLeftButtonMode = .back,
                         completion: @escaping Result<UIViewController, NSError>.Callback)
    {
                      ...
                      alternativeTitle: String? = nil, completion: (() -> Void)?)
    {
        if useSafari == true {
            ...
            webBrowserModule.onClose = { [weak self] _ in
            ...
            present(module: webBrowserModule) { [weak self] _ in
    ...
    fileprivate func removeViewControllersTail() {
        guard let navigationController = navigationController, self.viewControllers.count > 1 else {
            return
        }
        var viewControllersTail = viewControllers
    ...
    func makeNavigationBarTransparent() {
        if let navigationController = navigationController {
            navigationBarBackgroundImage = navigationController.navigationBar.backgroundImage(for: UIBarMetrics.default)
            navigationBarShadowImage = navigationController.navigationBar.shadowImage
            navigationBarIsTranslucent = navigationController.navigationBar.isTranslucent
            navigationBarViewBackgroundColor = navigationController.view.backgroundColor
            navigationBarBackgroundColor = navigationController.navigationBar.backgroundColor
            navigationBarTransparentStyleApplied = true
    ...
    func restoreNavigationBarFromTransparentState() {
        if let navigationController = navigationController, self.navigationBarTransparentStyleApplied == true {
            ...
            navigationController.view.backgroundColor = navigationBarViewBackgroundColor
