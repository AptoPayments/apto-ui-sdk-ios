...
public class RangeSlider: UIControl {
    ...
    override public func layoutSubviews() {
        ...
        maximumPosition = positionForValue(maximumValue)
        minimumPosition = positionForValue(minimumValue)
        updateLayerFrames()
    }
    ...
    func updateLayerFrames() {
        ...
        let lowerThumbCenter = currentlocation.x
    ...
    func positionForValue(_ value: Int) -> CGFloat {
        ...
        let dMaximumValue = Double(maximumValue)
        let dMinimumValue = Double(minimumValue)
    ...
    override public func beginTracking(_ touch: UITouch, with _: UIEvent?) -> Bool {
        ...
        if lowerThumbLayer.frame.contains(touch.location(in: self)) {
            ...
            delegate?.didStartSwippingIn(rangeSlider: self)
    ...
    override public func endTracking(_: UITouch?, with _: UIEvent?) {
        ...
        delegate?.didFinishSwippingIn(rangeSlider: self)
    }

    fileprivate func updateValueForCurrentlocation() {
        if currentlocation.x > maximumPosition {
            currentlocation.x = maximumPosition
        }
        if currentlocation.x < minimumPosition {
            currentlocation.x = minimumPosition
        }
        var newValue = valueForPosition(currentlocation.x - thumbWidth / 2)
        if minStep > 0 {
            newValue -= newValue % minStep
        }
