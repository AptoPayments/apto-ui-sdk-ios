...
open class CellController {
    ...
    func cell(_ tableView: UITableView) -> UITableViewCell {
        let cell = dequeueCellFrom(tableView: tableView)
        cellInstance = cell
        setupCell(cell)
        return cell
    }
    ...
    func dequeueCellFrom(tableView: UITableView) -> UITableViewCell {
        guard let identificator = reuseIdentificator() else {
            return UITableViewCell()
        }
        guard let cell = tableView.dequeueReusableCell(withIdentifier: identificator) else {
            registerCellInTableView(tableView: tableView)
            guard let cell = tableView.dequeueReusableCell(withIdentifier: identificator) else {
                return UITableViewCell()
            }
            return cell
    ...
    func registerCellInTableView(tableView: UITableView) {
        guard let identificator = reuseIdentificator(),
              let clazz = cellClass()
        else {
            return
        }
    ...
    func setupCell(_: UITableViewCell) {
        // Implement in subclasses
    }
}
