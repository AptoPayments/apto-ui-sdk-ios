...
class PhoneTextField: PhoneTextFieldView {
                ...
                uiConfig: UIConfig)
    {
        ...
        countryTextField = ComponentCatalog.formFieldWith(placeholder: "",
                                                          value: country.flag + "+" + String(countryCode),
                                                          uiConfig: uiConfig)
        countryPicker = CountryPicker(allowedCountries: allowedCountries,
                                      selectedCountry: country,
                                      showFlag: true,
                                      showPhoneCode: true,
                                      uiConfig: uiConfig)
        countryTextField.inputView = countryPicker
        phoneNumberField = ComponentCatalog.formPhoneTextFieldWith(placeholder: placeholder,
                                                                   value: nil,
                                                                   uiConfig: uiConfig)
...
private extension PhoneTextField {
    ...
    func set(initialPhoneNumber: InternationalPhoneNumber?) {
        if let value = initialPhoneNumber {
            ...
            {
                countryPicker.bndValue.send(country)
            }
            phoneNumberField.text = value.phoneNumber
        } else if let regionCode = Locale.current.regionCode,
                  let country = allowedCountries.first(where: { $0.isoCode == regionCode })
        {
            countryPicker.bndValue.send(country)
        } else {
            countryPicker.bndValue.send(allowedCountries[0])
        }
    }
}
