...
protocol ContentPresenterViewDelegate: AnyObject {
...
class ContentPresenterView: UIView {
    ...
    init(uiConfig: UIConfig) {
        ...
        font = uiConfig.fontProvider.instructionsFont
        textColor = uiConfig.textPrimaryColor
        linkColor = uiConfig.uiPrimaryColor
        waitListView = WaitListView(uiConfig: uiConfig, isAssetOptional: true)
    ...
    func set(content: Content) {
        ...
        case let .externalURL(url):
            show(url)
        case let .nativeContent(nativeContent):
            show(nativeContent)
        }
    }
