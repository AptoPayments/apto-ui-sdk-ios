...
class VoIPViewController: AptoViewController {
    ...
    init(uiConfiguration: UIConfig, presenter: VoIPPresenterProtocol) {
        ...
        titleLabel = ComponentCatalog.topBarTitleLabelWith(text: "manage_card.get_pin_voip.title".podLocalized(),
                                                           uiConfig: uiConfiguration)
        let message = "manage_card.get_pin_voip.message".podLocalized()
        messageLabel = ComponentCatalog.mainItemRegularLabelWith(text: message, textAlignment: .center,
                                                                 uiConfig: uiConfiguration)
        actionsView = CallActionButtonsView(uiConfig: uiConfiguration)
        hangupButton = KeyboardButton(type: .image(normalStateConfig: hangupButtonConfig))
...
extension VoIPViewController: CallActionButtonsViewDelegate {
    ...
    func keypadButtonTapped(sender _: KeyboardButton) {
        toggleKeyboard()
    }
}
