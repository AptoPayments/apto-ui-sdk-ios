...
class FormRowRadioView: FormRowMultilineView {
    ...
    var numberValidator: DataValidator<Int>? {
        didSet {
            validateInt(numberValidator, number: selectedValue)
        }
    }
         ...
         flashColor: UIColor?)
    {
        ...
        add(lines: lines)
        if onImage != nil {
            tickImageView = UIImageView(image: onImage)
        } else {
            tickImageView = UIImageView(image: UIImage.imageFromPodBundle("activation_check_blue.png")?.asTemplate())
        }
    }
    ...
    var bndValue: Observable<Int?> {
        if let bndValue = _bndValue {
            ...
            let bndValue = Observable<Int?>(selectedValue)
    ...
    fileprivate func hideCurrentTick() {
        selectedLine = nil
        guard let tickImageView = tickImageView else {
            return
        }
