...
class SecretPINView: UIView {
  ...
  weak var delegate: SecretPINViewDelegate?
  ...
  private func digitButtonTapped(_ sender: UIButton) {
    ...
    if let title = sender.title(for: .normal) {
      ...
      if code.count == codeLength {
        delegate?.secretPINView(self, didEnterPIN: code)
      }
    }
  }
...
// MARK: - Set up UI
private extension SecretPINView {
  func setUpUI() {
  ...
  func createBottomRow() -> UIView {
    ...
    addSubview(container)
    container.snp.makeConstraints { make in
      make.left.bottom.right.equalToSuperview().inset(48)
    }
