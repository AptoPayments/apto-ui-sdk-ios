...
extension AptoPlatform {
    ...
    /// - Parameter options: A set of flags that control whether some functions are available
    /// or not in the different screens.
    /// - Parameter initialUserData: User data that will be used in the sign up flow. If it's not provided,
    /// the SDK will ask for that data
    ...
    /// - Parameter options: A set of flags that control whether some functions are available
    /// or not in the different screens.
    /// - Parameter initialUserData: User data that will be used in the sign up flow.
    /// If it's not provided, the SDK will ask for that data
    /// - Parameter initializationData: a `InitializationData` object that wraps additional user data. It can be nil.
    /// - Parameter googleMapsApiKey: The google maps api key that will be used to verify the user address.
    ...
    @objc public func startNewCardApplicationFlow(from: UIViewController,
                                                  ...
                                                  initializationData: InitializationData? = nil,
                                                  googleMapsApiKey: String?,
                                                  completion: @escaping (_ module: UIModule?,
    ...
    private func setUpUINotificationObservers() {
        ...
        notificationHandler.addObserver(self, selector: #selector(didRestoreNetworkConnectionUI),
                                        name: .NetworkReachableNotification)
        notificationHandler.addObserver(self, selector: #selector(didLoseNetworkConnectionUI),
                                        name: .NetworkNotReachableNotification)
        notificationHandler.addObserver(self, selector: #selector(didLoseConnectionToServerUI),
                                ...
                                completion: @escaping (Result<UIModule, NSError>.Callback))
    {
        ...
        fetchContextConfiguration { [weak self] result in
            ...
            case let .success(contextConfiguration):
                ...
                cardModule.onClose = { [weak self, weak from] _ in
